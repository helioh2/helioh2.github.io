...
Blockly.JavaScript['text_multiline'] = function(block) {
  // Text value.
  const code =
      Blockly.JavaScript.multiline_quote_(block.getFieldValue('TEXT'));
...
Blockly.JavaScript['text_charAt'] = function(block) {
  ...
  const textOrder = (where === 'RANDOM') ? Blockly.JavaScript.ORDER_NONE :
      Blockly.JavaScript.ORDER_MEMBER;
  const text =
      Blockly.JavaScript.valueToCode(block, 'VALUE', textOrder) || "''";
  switch (where) {
    case 'FIRST': {
...
Blockly.JavaScript['text_getSubstring'] = function(block) {
  ...
  const textOrder = requiresLengthCall ? Blockly.JavaScript.ORDER_MEMBER :
      Blockly.JavaScript.ORDER_NONE;
  const text =
      Blockly.JavaScript.valueToCode(block, 'STRING', textOrder) || "''";
  let code;
...
function ${Blockly.JavaScript.FUNCTION_NAME_PLACEHOLDER_}(sequence${at1Param}${at2Param}) {
...
Blockly.JavaScript['text_changeCase'] = function(block) {
  ...
  const text =
      Blockly.JavaScript.valueToCode(block, 'TEXT', textOrder) || "''";
  let code;
  if (operator) {
    // Upper and lower case are functions built into Blockly.JavaScript.
    code = text + operator;
  } else {
...
Blockly.JavaScript['text_count'] = function(block) {
  ...
  const sub = Blockly.JavaScript.valueToCode(block, 'SUB',
      Blockly.JavaScript.ORDER_NONE) || "''";
  const functionName = Blockly.JavaScript.provideFunction_('textCount', `
...
Blockly.JavaScript['text_replace'] = function(block) {
  ...
  const functionName = Blockly.JavaScript.provideFunction_('textReplace', `
